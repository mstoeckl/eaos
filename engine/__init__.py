...
class Engine():
    ...
    def refresh_robot(self, robot):
        ...
        for match in self.data.get_matches(robot):
            table.append(
                [match] + self.year.get_robot_match_results(robot, match))

        self.output(
            "/view/robot/" + robot, "update_matches", json.dumps(table))
    ...
    def refresh_match(self, match):
        ...
        for term in ("Red", "Blue"):
            ...
            fields[
                term + "_score_total"] = self.data.get_total_score(match, term)
    ...
    def receive(self, page, key, val):
        ...
        if page == "/input/schedule":
            if key in ("csv_entry", "csv_reset"):
                if key == "csv_entry":
                    table, deleted_matches, deleted_robots = self.data.update_schedule(
                        val)
                else:
                ...
                self.output(
                    "/input/actions", "match_list", self.data.matchoptions())
                self.spread(["/input/match", "/view/match"],
                            "match_list", json.dumps(self.data.matchlist()))
                self.output(
                    "/view/robot", "robot_list", json.dumps(self.data.robotlist()))
        ...
        if page == "/input/actions":
            ...
            if key == "match_num":
                for i in range(6):
                    self.output(
                        page, "team-{}-{}".format(i, val), self.data.get_actions_team(val, i))
                    self.output(
                        page, "actions-{}-{}".format(i, val), self.data.get_actions_actions(val, i))
                    self.output(
                        page, "scouter-{}-{}".format(i, val), self.data.get_actions_scouter(val, i))
                return
            ...
            if key == "request_scores":
                for match in self.data.matchlist():
                    for key in Store.scores:
                        self.output(
                            page, key + "-" + match, self.data.get_score(match, key))
                return
            elif "-" in key:
                ...
                if item.startswith("Red"):
                    ...
                    term = "Blue"
                self.output("/view/match/" + match, term +
                            "_score_total", self.data.get_total_score(match, term))
